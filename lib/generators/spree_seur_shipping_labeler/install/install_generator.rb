module SpreeSeurShippingLabeler
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def add_stylesheets
        inject_into_file 'vendor/assets/stylesheets/spree/frontend/all.css', " *= require spree/frontend/spree_seur_shipping_labeler\n", :before => /\*\//, :verbose => true
        inject_into_file 'vendor/assets/stylesheets/spree/backend/all.css', " *= require spree/backend/spree_seur_shipping_labeler\n", :before => /\*\//, :verbose => true
      end
      ...
      def run_tasks
        run_tasks = options[:auto_run_tasks] || ['', 'y', 'Y'].include?(ask 'Would you like to run the default seur shipping types now? [Y/n]')
        if run_tasks
          run 'bundle exec rake shipping_seur_types:create'
        else
          puts 'Skipping shipping seur types, don\'t forget to run it!'
        end
      end
    end
  end
